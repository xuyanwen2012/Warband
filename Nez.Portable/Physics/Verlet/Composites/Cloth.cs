...
namespace Nez.Verlet
{
    public class Cloth : Composite
    {
        ...
        public Cloth(
            ...
            float tearSensitivity = 5,
            bool connectHorizontalParticles = true)
        {
                    ...
                    if (connectHorizontalParticles && x > 0)
                        addConstraint(
                            new DistanceConstraint(
                                particles[y * segments + x],
                                particles[y * segments + x - 1],
                                stiffness)).setTearSensitivity(tearSensitivity).setCollidesWithColliders(false);

                    if (y > 0)
                        addConstraint(
                            new DistanceConstraint(
                                particles[y * segments + x],
                                particles[(y - 1) * segments + x],
                                stiffness)).setTearSensitivity(tearSensitivity).setCollidesWithColliders(false);
