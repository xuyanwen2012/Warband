...
namespace Nez.Tiled
{
    public class TiledTileLayer : TiledLayer
    {
        ...
        public TiledTileLayer(TiledMap map, string name, int width, int height)
            : this(map, name, width, height, new TiledTile[width * height])
        {
        }
        ...
        public override void draw(Batcher batcher, Vector2 position, float layerDepth, RectangleF cameraClipBounds)
        {
            ...
            if (tiledMap.requiresLargeTileCulling)
            {
                ...
                minX = tiledMap.worldToTilePositionX(
                    cameraClipBounds.left - (tiledMap.largestTileWidth - tiledMap.tileWidth));
                minY = tiledMap.worldToTilePositionY(
                    cameraClipBounds.top - (tiledMap.largestTileHeight - tiledMap.tileHeight));
                maxX = tiledMap.worldToTilePositionX(
                    cameraClipBounds.right + (tiledMap.largestTileWidth - tiledMap.tileWidth));
                maxY = tiledMap.worldToTilePositionY(
                    cameraClipBounds.bottom + (tiledMap.largestTileHeight - tiledMap.tileHeight));
            }
            else
            {
 ...
 // end for x
        ...
        public Rectangle findBoundsRect(int startX, int endX, int startY, bool?[] checkedIndexes)
        {
            ...
            return new Rectangle(
                ...
                startY * tiledMap.tileHeight,
                (endX - startX) * tiledMap.tileWidth,
                (tiledMap.height - startY) * tiledMap.tileHeight);
        }
