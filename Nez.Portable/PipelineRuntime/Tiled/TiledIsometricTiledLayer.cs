...
namespace Nez.Tiled
{
    public class TiledIsometricTiledLayer : TiledLayer
    {
        ...
        public TiledIsometricTiledLayer(TiledMap map, string name, int width, int height)
            ...
            for (var y = min.Y; y <= max.Y; y++)
            {
                for (var x = min.X; x <= max.X; x++)
                {
                    ...
                    if (tileworldpos.X < cameraClipBounds.left || tileworldpos.Y < cameraClipBounds.top
                                                               || tileworldpos.X > cameraClipBounds.right
                                                               || tileworldpos.Y > cameraClipBounds.bottom)
                        continue;
