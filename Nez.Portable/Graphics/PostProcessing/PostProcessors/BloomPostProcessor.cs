...
namespace Nez
{
    public class BloomPostProcessor : PostProcessor
    {
        ...
        EffectParameter _bloomBaseIntensityParam;

        EffectParameter _bloomSaturationParam;

        EffectParameter _bloomBaseSaturationParam;

        EffectParameter _bloomBaseMapParm;

        public BloomPostProcessor(int executionOrder)
            : base(executionOrder)
        {
        ...
        public override void process(RenderTarget2D source, RenderTarget2D destination)
        {
            ...
            var sceneRenderTargetSize = scene.sceneRenderTargetSize;
            var renderTarget1 = RenderTarget.getTemporary(
                (int)(sceneRenderTargetSize.X * renderTargetScale),
                (int)(sceneRenderTargetSize.Y * renderTargetScale),
                DepthFormat.None);
            var renderTarget2 = RenderTarget.getTemporary(
                (int)(sceneRenderTargetSize.X * renderTargetScale),
                (int)(sceneRenderTargetSize.Y * renderTargetScale),
                DepthFormat.None);
