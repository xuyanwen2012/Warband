        ...
        public Vector2[] getDrawingPoints(List<SvgPathSegment> segments, float flatness = 3)
        {
            ...
            for (var j = 0; j < segments.Count; j++)
            {
                ...
                else if (segment is SvgCubicCurveSegment)
                {
                    ...
                    path.AddBezier(
                        toDrawPoint(segment.start),
                        toDrawPoint(cubicSegment.firstCtrlPoint),
                        toDrawPoint(cubicSegment.secondCtrlPoint),
                        toDrawPoint(segment.end));
                }
                ...
                else if (segment is SvgQuadraticCurveSegment)
                {
                    ...
                    path.AddBezier(
                        toDrawPoint(segment.start),
                        toDrawPoint(quadSegment.firstCtrlPoint),
                        toDrawPoint(quadSegment.secondCtrlPoint),
                        toDrawPoint(segment.end));
                }
                else
                {
            ...
            var matrix = System.Activator.CreateInstance(
                System.Type.GetType("System.Drawing.Drawing2D.Matrix, System.Drawing"));
