...
namespace Nez.DeferredLighting
{
    public class DeferredLightEffect : Effect
    {
        ...
        public DeferredLightEffect()
            : base(Core.graphicsDevice, EffectResource.deferredLightBytes)
        {
        ...
        public void updateForLight(PointLight light)
        {
            ...
            var objToWorld = Matrix.CreateScale(light.radius * light.entity.transform.scale.X)
                             * Matrix.CreateTranslation(
                                 light.entity.transform.position.X + light.localOffset.X,
                                 light.entity.transform.position.Y + light.localOffset.Y,
                                 0);
        ...
        public void updateForLight(AreaLight light)
        {
            ...
            var objToWorld =
                Matrix.CreateScale(
                    light.bounds.width * light.entity.transform.scale.X,
                    light.bounds.height * light.entity.transform.scale.Y,
                    1f) * Matrix.CreateTranslation(
                    light.bounds.x - light.bounds.width * 0.5f,
                    light.bounds.y - light.bounds.height * 0.5f,
                    0);
            setObjectToWorldMatrix(objToWorld);
