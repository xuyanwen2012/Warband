...
namespace Nez.UI
{
    ...
    public class TextField : Element, IInputListener, IKeyboardListener
    {
        ...
        public override float preferredHeight
        {
            get
            {
                ...
                if (style.background != null)
                    prefHeight = Math.Max(
                        prefHeight + style.background.bottomHeight + style.background.topHeight,
        ...
        public override void draw(Graphics graphics, float parentAlpha)
        {
            ...
            if (displayText.Length == 0)
            {
                if (!_isFocused && messageText != null)
                {
                    ...
                    graphics.batcher.drawString(
                        messageFont,
                        messageText,
                        new Vector2(x + bgLeftWidth, y + textY + yOffset),
                        messageFontColor);
            ...
            else
            {
                ...
                graphics.batcher.drawString(
                    font,
                    t,
                    new Vector2(x + bgLeftWidth + textOffset, y + textY + yOffset),
                    col);
            }
        ...
        protected void drawSelection(IDrawable selection, Graphics graphics, BitmapFont font, float x, float y)
        {
            selection.draw(
                graphics,
                x + selectionX + renderOffset + fontOffset,
                y - font.descent / 2,
                selectionWidth,
                textHeight,
                Color.White);
            ...
            bool up)
        {
            ...
            for (int i = 0, n = elements.Count; i < n; i++)
            {
                ...
                if (element is TextField)
                {
                    ...
                    {
                        if (best == null || (elementCoords.Y > bestCoords.Y
                                             || (elementCoords.Y == bestCoords.Y && elementCoords.X < bestCoords.X))
                            ^ up)
                        {
