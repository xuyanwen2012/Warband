...
namespace Nez.UI
{
    ...
    public class NinePatchDrawable : IDrawable
    {
        ...
        public NinePatchDrawable(Subtexture subtexture, int left, int right, int top, int bottom)
            : this(new NinePatchSubtexture(subtexture.texture2D, subtexture.sourceRect, left, right, top, bottom))
        {
        }
        ...
        public void draw(Graphics graphics, float x, float y, float width, float height, Color color)
        {
            ...
            if (_finalRenderRect.Height != height || _finalRenderRect.Width != width)
            {
                ...
                _subtexture.generateNinePatchRects(
                    _finalRenderRect,
                    _destRects,
                    _subtexture.left,
                    _subtexture.right,
