...
namespace Nez.UI
{
    ...
    public class Stage
    {
        ...
        bool debugUnderMouse;

        bool debugParentUnderMouse;
        ...
        void updateInputMoved(Vector2 inputPos, Element over, Element lastOver)
        {
            for (var i = _inputFocusListeners.Count - 1; i >= 0; i--)
                ((IInputListener)_inputFocusListeners[i]).onMouseMoved(
                    _inputFocusListeners[i].stageToLocalCoordinates(inputPos));
        ...
        {
            for (var i = _inputFocusListeners.Count - 1; i >= 0; i--)
                ((IInputListener)_inputFocusListeners[i]).onMouseUp(
                    _inputFocusListeners[i].stageToLocalCoordinates(inputPos));
                                    ...
                                    t =>
                                        {
                                            ...
                                            if (self._keyboardFocusElement != null)
                                                self._keyboardFocusElement.keyPressed(
                                                    _repeatKey,
                                                    _repeatKey.getChar().Value);
                                        });
                            }
                        }
                    }
        ...
        public List<T> findAllElementsOfType<T>()
            where T : class
        {
            var eles = new List<T>();
        ...
        void findAllElementsOfType<T>(List<Element> elements, List<T> foundElements)
            where T : class
        {
