...
namespace Nez.UI
{
    ...
    public class ScrollPane : Group, IInputListener
    {
        ...
        float _fadeAlphaSeconds = 0.5f;

        float _fadeDelay;

        float _fadeDelaySeconds = 1f;
        ...
        bool _overscrollX = true;

        bool _overscrollY = true;

        float _flingTime = 1f;

        float _overscrollDistance = 50;

        float _overscrollSpeedMin = 30;

        float _overscrollSpeedMax = 200;

        bool _forceScrollX;

        bool _forceScrollY;

        protected bool _disableX = true;

        protected bool _disableY;
        ...
        public ScrollPane(Element widget, Skin skin)
            : this(widget, skin.get<ScrollPaneStyle>())
        {
        }

        public ScrollPane(Element widget, Skin skin, string styleName)
        ...
        public override void layout()
        {
            ...
            if (_scrollX)
            {
                if (hScrollKnob != null)
                {
                    ...
                    if (_variableSizeKnobs)
                        _hKnobBounds.Width = (int)Math.Max(
                            hScrollKnob.minWidth,
            ...
            if (_scrollY)
            {
                if (vScrollKnob != null)
                {
                    ...
                    if (_variableSizeKnobs)
                        _vKnobBounds.Height = (int)Math.Max(
                            vScrollKnob.minHeight,
        ...
        protected virtual void update()
        {
                ...
                if (_visualAmountX != _amountX)
                {
                    ...
                    if (_visualAmountX < _amountX)
                        setVisualScrollX(
                            Math.Min(
                                _amountX,
                                _visualAmountX + Math.Max(
                                    2000 * Time.unscaledDeltaTime,
            ...
            if (_overscrollX && _scrollX)
            {
                ...
                {
                    resetFade();
                    _amountX -= (_overscrollSpeedMin + (_overscrollSpeedMax - _overscrollSpeedMin) * -(_maxX - _amountX)
            ...
            if (_overscrollY && _scrollY)
            {
                ...
                {
                    resetFade();
                    _amountY -= (_overscrollSpeedMin + (_overscrollSpeedMax - _overscrollSpeedMin) * -(_maxY - _amountY)
                                 / _overscrollDistance) * Time.unscaledDeltaTime;
                    if (_amountY < _maxY)
                        setScrollY(_maxY);
                }
    ...
    public class ScrollPaneStyle
    {
        ...
        public IDrawable corner;

        /** Optional. */
        public IDrawable hScroll;

        public IDrawable hScrollKnob;

        /** Optional. */
        public IDrawable vScroll;

        public IDrawable vScrollKnob;
