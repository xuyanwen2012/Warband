...
namespace Nez.UI
{
    ...
    public class Table : Group
    {
        ...
        static float[] _columnWeightedWidth;

        static float[] _rowWeightedHeight;
        ...
        float[] _columnMinWidth;

        float[] _rowMinHeight;

        float[] _columnPrefWidth;

        float[] _rowPrefHeight;

        float _tableMinWidth;

        float _tableMinHeight;

        float _tablePrefWidth;

        float _tablePrefHeight;
        ...
        Value _padLeft = backgroundLeft;

        Value _padBottom = backgroundBottom;

        Value _padRight = backgroundRight;
        ...
        void layout(float layoutX, float layoutY, float layoutWidth, float layoutHeight)
        {
            ...
            for (var i = 0; i < cellCount; i++)
            {
                ...
                cell.elementWidth = Math.Min(
                    spannedWeightedWidth - cell.computedPadLeft - cell.computedPadRight,
                    prefWidth);
                cell.elementHeight = Math.Min(
                    weightedHeight - cell.computedPadTop - cell.computedPadBottom,
                    prefHeight);
            ...
            float vpadding)
        {
            ...
            for (var i = 0; i < _cells.Count; i++)
            {
                ...
                if (_tableDebug == TableDebug.Cell || _tableDebug == TableDebug.All)
                {
                    addDebugRect(
                        currentX,
                        currentY + cell.computedPadTop,
                        spannedCellWidth,
                        _rowHeight[cell.row] - cell.computedPadTop - cell.computedPadBottom,
                        debugCellColor);
                }
