...
namespace Nez.UI
{
    public class Skin
    {
        ...
        public static Skin createDefaultSkin()
        {
            ...
            var windowStyle = new WindowStyle { background = new PrimitiveDrawable(windowColor) };
            skin.add("default", windowStyle);

            var textFieldStyle = TextFieldStyle.create(
                textFieldFontColor,
            ...
            var scrollPaneStyle = new ScrollPaneStyle
                                      {
                                          vScroll =
                                              new PrimitiveDrawable(6, 0, scrollPaneScrollBarColor),
                                          vScrollKnob =
                                              new PrimitiveDrawable(6, 50, scrollPaneKnobColor),
                                          hScroll = new PrimitiveDrawable(
                                              0,
                                              6,
                                              scrollPaneScrollBarColor),
                                          hScrollKnob = new PrimitiveDrawable(
                                              50,
                                              6,
                                              scrollPaneKnobColor)
                                      };
            ...
            var listBoxStyle = new ListBoxStyle
                                   {
                                       ...
                                       hoverSelection =
                                           new PrimitiveDrawable(listBoxHoverSelectionColor, 5, 5),
            ...
            var selectBoxStyle = new SelectBoxStyle
                                     {
                                         ...
                                         background = new PrimitiveDrawable(
                                             selectBoxBackgroundColor,
                                             4,
                                             4)
                                     };
                ...
                for (var i = 0; i < styleClasses.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        for (var j = 0; j < styleNames.Count; j++)
                        {
                            ...
                            style = setStylesForStyleClassMethod.Invoke(
                                this,
        ...
        public T setStylesForStyleClass<T>(
            T styleClass,
            Dictionary<string, object> styleDict,
            ...
            string styleName)
        {
            foreach (var styleConfig in styleDict)
            {
                ...
                {
                    ReflectionUtils.getFieldInfo(styleClass, name).SetValue(
                        styleClass,
                        contentManager.Load<BitmapFont>(identifier));
                }
                else if (name.ToLower().EndsWith("style"))
                {
                    ...
                    if (valueObject is Dictionary<string, object>)
                    {
                        ...
                        inlineStyle = setStylesForStyleClassMethod.Invoke(
                            this,
                            new object[]
                    ...
                    else
                    {
                        ...
                        if (theStyle == null)
                            Debug.error(
                                "could not find a style reference named {0} when setting {1} on {2}",
                                identifier,
                                name,
                                styleName);
                    }
                ...
                else
                {
                    ...
                    if (drawable != null)
                        ...
                        Debug.error(
                            "could not find a drawable or color named {0} when setting {1} on {2}",
                            identifier,
                            name,
                            styleName);
                }
