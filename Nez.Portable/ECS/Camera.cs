...
namespace Nez
{
    public class Camera : Component
    {
        ...
        public RectangleF bounds
        {
            get
            {
                ...
                if (_areBoundsDirty)
                {
                    ...
                    var topLeft = screenToWorldPoint(
                        new Vector2(
                            Core.graphicsDevice.Viewport.X + _inset.left,
                            Core.graphicsDevice.Viewport.Y + _inset.top));
                    var bottomRight = screenToWorldPoint(
                        new Vector2(
                    ...
                    if (entity.transform.rotation != 0)
                    {
                        ...
                        var topRight = screenToWorldPoint(
                            new Vector2(
                                Core.graphicsDevice.Viewport.X + Core.graphicsDevice.Viewport.Width - _inset.right,
                                Core.graphicsDevice.Viewport.Y + _inset.top));
                        var bottomLeft = screenToWorldPoint(
                            new Vector2(
                                Core.graphicsDevice.Viewport.X + _inset.left,
                                Core.graphicsDevice.Viewport.Y + Core.graphicsDevice.Viewport.Height - _inset.bottom));
        ...
        public Matrix viewProjectionMatrix
        {
            get
            {
                return transformMatrix * projectionMatrix;
            }
        }
        ...
        protected virtual void updateMatrixes()
        {
            ...
            _transformMatrix = Matrix2D.createTranslation(
                ...
                -entity.transform.position.Y); // position
