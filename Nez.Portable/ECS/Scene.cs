...
namespace Nez
{
    public class Scene
    {
        ...
        public Point sceneRenderTargetSize
        {
            get
            {
                return new Point(_sceneRenderTarget.Bounds.Width, _sceneRenderTarget.Bounds.Height);
            }
        }
        ...
        public IFinalRenderDelegate finalRenderDelegate
        {
            ...
            set
            {
                ...
                this._finalRenderDelegate.scene = this;
                this._finalRenderDelegate.onAddedToScene();
            }
        }
        ...
        public static T createWithDefaultRenderer<T>(Color? clearColor = null)
            where T : Scene, new()
        {
            var scene = new T();
        ...
        /// <returns>The with default renderer.</returns>
        [Obsolete("use new Scene() instead")]
        public static T create<T>(Color? clearColor = null)
            where T : Scene, new()
        {
            var scene = new T();
        ...
        internal void postRender(RenderTarget2D finalRenderTarget = null)
        {
            ...
            if (enablePostProcessing)
            {
                for (var i = 0; i < _postProcessors.length; i++)
                {
                    if (_postProcessors.buffer[i].enabled)
                    {
                        ...
                        enabledCounter++;
                        _postProcessors.buffer[i].process(
                            isEven ? _sceneRenderTarget : _destinationRenderTarget,
            ...
            for (var i = 0; i < _afterPostProcessorRenderers.length; i++)
            {
                if (i == 0)
                {
                    ...
                    Core.graphicsDevice.setRenderTarget(
            ...
            {
                _finalRenderDelegate.handleFinalRender(
                    letterboxColor,
                    Mathf.isEven(enabledCounter) ? _sceneRenderTarget : _destinationRenderTarget,
                    _finalRenderDestinationRect,
                    samplerState);
            }
            else
            {
        ...
        void updateResolutionScaler()
        {
            ...
            if (!rectCalculated)
            {
                ...
                _finalRenderDestinationRect = RectangleExt.fromFloats(
                    (screenSize.X - renderWidth) / 2,
                    (screenSize.Y - renderHeight) / 2,
                    renderWidth,
                    renderHeight);
            }
        ...
        /// <returns>The component.</returns>
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        public T getOrCreateSceneComponent<T>()
            where T : SceneComponent, new()
        {
        ...
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        public bool removeSceneComponent<T>()
            where T : SceneComponent
        {
            var comp = getSceneComponent<T>();
            if (comp != null)
        ...
        public T addPostProcessor<T>(T postProcessor)
            where T : PostProcessor
        {
        ...
        public Entity addEntity(Entity entity)
        {
            Assert.isFalse(
                ...
                "You are attempting to add the same entity to a scene twice: {0}",
                entity);
            entities.add(entity);
            entity.scene = this;
