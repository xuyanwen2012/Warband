...
namespace Nez.Tiled
{
...
#endif
    {
            ...
            out int collisionResponse)
        {
            ...
            for (var i = 0; i < _collidingTiles.Count; i++)
            {
                ...
                if (direction.isHorizontal() && collisionState._lastGroundTile != null
                                             && collisionState._lastGroundTile.isSlope()
                                             && isSlopeCollisionRow(_collidingTiles[i].y))
                    continue;

                if (testTileCollision(
                    _collidingTiles[i],
                    side,
                    perpindicularPosition,
                    leadingPosition,
                    shouldTestSlopes,
                    out collisionResponse))
                {
        ...
        bool testTileCollision(
            TiledTile tile,
            Edge edgeToTest,
            int perpindicularPosition,
            int leadingPosition,
            bool shouldTestSlopes,
            out int collisionResponse)
        {
