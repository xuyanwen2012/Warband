    ...
    public class LineRenderer : RenderableComponent
    {
        ...
        void calculateVertices()
        {
            ...
            for (var i = 0; i < _points.length - 1; i++)
            {
                ...
                maxX = Mathf.maxOf(
                    maxX,
                    _points.buffer[i].position.X + halfMaxWidth,
                ...
                maxY = Mathf.maxOf(
                    maxY,
                    _points.buffer[i].position.Y + halfMaxWidth,
            ...
            for (var i = 0; i < _points.length - 1; i++)
            {
                ...
                if (hasThirdPoint)
                {
                    var shouldFuseBottom = Vector2Ext.isTriangleCCW(
                        firstPoint.position,
        ...
        void addFirstSegment(ref Segment segment, ref Segment nextSegment, ref int vertIndex)
        {
            ...
            if (nextSegment.shouldFuseBottom)
            {
                ...
                addVert(
                    vertIndex++,
                    nextSegment.hasFusedPoint ? nextSegment.fusedPoint : segment.tl,
                    new Vector2(1, 0),
            ...
            else
            {
                addVert(
                    vertIndex++,
                    nextSegment.hasFusedPoint ? nextSegment.fusedPoint : segment.bl,
                    new Vector2(1, 1),
                    segment.nextPoint.color);
                addVert(vertIndex++, nextSegment.point.position, new Vector2(1, 0.5f), segment.nextPoint.color);
        ...
        void addSegment(ref Segment segment, ref int vertIndex)
        {
            ...
            if (segment.shouldFuseBottom)
            {
                ...
                addVert(
                    vertIndex++,
                    segment.hasFusedPoint ? segment.fusedPoint : segment.bl,
                    new Vector2(0, 0),
                    segment.point.color);
            }
            else
            {
                addVert(
                    vertIndex++,
                    segment.hasFusedPoint ? segment.fusedPoint : segment.tl,
                    new Vector2(0, 1),
                    segment.point.color);
                addVert(vertIndex++, segment.tr, new Vector2(1, 1), segment.nextPoint.color);
                addVert(vertIndex++, segment.br, new Vector2(1, 0), segment.nextPoint.color);
