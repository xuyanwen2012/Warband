...
namespace Nez.Shadows
{
    ...
    public class PolySpotLight : PolyLight
    {
        ...
        public PolySpotLight(float radius)
            : this(radius, Color.White)
        {
        }

        public PolySpotLight(float radius, Color color)
            : this(radius, color, 1.0f)
        {
        }

        public PolySpotLight(float radius, Color color, float power)
            : base(radius, color, power)
        {
        }
        ...
        void recalculatePolyPoints()
        {
            ...
            for (var i = 0; i <= sides; i++)
                verts[i + 1] = new Vector2(
                    expandedRadius * Mathf.cos(stepSize * i),
            ...
            Polygon.rotatePolygonVerts(
                entity.rotation - _spotAngle * 0.5f * Mathf.deg2Rad,
                _polygon._originalPoints,
                _polygon.points);
        }
        ...
        public override void debugRender(Graphics graphics)
        {
            ...
            graphics.batcher.drawPolygon(
                _polygon.position,
                _polygon.points,
                Debug.Colors.colliderEdge,
                true,
