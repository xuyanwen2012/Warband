...
namespace Nez.Particles
{
    public class ParticleEmitter : RenderableComponent, IUpdatable
    {
        ...
        public bool simulateInWorldSpace
        {
            set
            {
                _emitterConfig.simulateInWorldSpace = value;
            }
        }
        ...
        void IUpdatable.update()
        {
            ...
            else
            {
                ...
                _bounds.inflate(
                    _emitterConfig.subtexture.sourceRect.Width * maxParticleSize,
            ...
            for (var i = 0; i < _particles.Count; i++)
            {
                ...
                if (_emitterConfig.subtexture == null)
                    graphics.batcher.draw(
                        graphics.pixelTexture,
                        pos + currentParticle.position,
                        currentParticle.color,
                        currentParticle.rotation,
                        Vector2.One,
                        currentParticle.particleSize * 0.5f,
                        SpriteEffects.None,
                        layerDepth);
                else
                    graphics.batcher.draw(
                        _emitterConfig.subtexture,
                        pos + currentParticle.position,
                        currentParticle.color,
                        currentParticle.rotation,
                        _emitterConfig.subtexture.center,
                        currentParticle.particleSize / _emitterConfig.subtexture.sourceRect.Width,
                        SpriteEffects.None,
                        layerDepth);
            }
        }
