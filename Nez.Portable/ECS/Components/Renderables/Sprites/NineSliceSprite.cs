...
namespace Nez
{
    public class NineSliceSprite : Sprite
    {
        ...
        public new float height
        {
            get
            {
                return _finalRenderRect.Height;
            }
        ...
        public NineSliceSprite(NinePatchSubtexture subtexture)
            : base(subtexture)
        {
        ...
        public NineSliceSprite(Subtexture subtexture, int top, int bottom, int left, int right)
            : this(new NinePatchSubtexture(subtexture, left, right, top, bottom))
        {
        }

        public NineSliceSprite(Texture2D texture, int top, int bottom, int left, int right)
            : this(new NinePatchSubtexture(texture, left, right, top, bottom))
        {
        }
        ...
        public override void render(Graphics graphics, Camera camera)
        {
            if (_destRectsDirty)
            {
                subtexture.generateNinePatchRects(
                    _finalRenderRect,
                    _destRects,
                    subtexture.left,
                    subtexture.right,
