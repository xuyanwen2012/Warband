...
namespace Nez
{
    ...
    public class FollowCamera : Component, IUpdatable
    {
        ...
        public override void debugRender(Graphics graphics)
        {
            if (_cameraStyle == CameraStyle.LockOn)
                graphics.batcher.drawHollowRect(
                    _worldSpaceDeadzone.x - 5,
                    _worldSpaceDeadzone.y - 5,
                    _worldSpaceDeadzone.width,
                    _worldSpaceDeadzone.height,
                    Color.DarkRed);
            else
        ...
        public void setCenteredDeadzone(int width, int height)
        {
            ...
            var cameraBounds = camera.bounds;
            deadzone = new RectangleF(
                (cameraBounds.width - width) / 2,
                (cameraBounds.height - height) / 2,
                width,
                height);
        }
    }
